...
const sendOTPEmail = require('../utils/sendEmail'); // existing util, if present

// ...existing code...

// Replace or add signup controller to accept name, gender, state, district, pincode and send OTP
exports.signup = async (req, res) => {
  try {
    const { email, name, gender, state, district, pincode } = req.body;
    if (!email || !name) return res.status(400).json({ message: 'Email and name are required.' });

    // create or update user record (not verified yet)
    let user = await User.findOne({ email });
    if (!user) {
      user = await User.create({ email, name, gender, state, district, pincode });
    } else {
    ...
    // store or update OTP (expires handled by schema TTL)
    ...
    try {
      if (typeof sendOTPEmail === 'function') {
        await sendOTPEmail(email, `Your verification OTP is ${code}`);
      }
    } catch (err) {
...
// Replace or add verify-otp controller to verify code and return JWT + user
exports.verifyOtp = async (req, res) => {
  try {
    ...
    // find user (should exist from signup)
    ...
    // return token and user (omit sensitive fields)
...
// // const Jimp = JimpModule && (typeof JimpModule.read === 'function' ? JimpModule : (JimpModule.default || JimpModule));
...
//       // pick correct read function for installed jimp version
//       // const jimpRead = (Jimp && typeof Jimp.read === 'function' && Jimp.read.bind(Jimp)) ||
//       //                  (Jimp && typeof Jimp.default?.read === 'function' && Jimp.default.read.bind(Jimp.default));

//       // if (!jimpRead) {
//       //   throw new Error('Jimp.read is not available. Install a compatible jimp (e.g. npm i jimp) or adjust import.');
//       // }

//       const image = await jimpRead(buffer);
//       const qrCodeInstance = new QrCode();
...
//             let decompressed;
//             try { decompressed = pako.inflate(dataToVerify); }
//             catch (e) { throw new Error('Decompression failed for secure QR payload.'); }
...
//           }
//         } catch (err) {
//           console.warn('Secure QR parse failed, falling back to XML if possible:', err.message || err);
//           parsed = null;
//         }
//       }
...
//       if (parsed.aadhaarFull && /^\d{12}$/.test(parsed.aadhaarFull)) user.aadhaarNumber = parsed.aadhaarFull;
//       else if (parsed.aadhaarPartial) user.aadhaarNumber = `********${parsed.aadhaarPartial}`;
//       await user.save();

//       return res.status(200).json({ message: 'Aadhaar details verified and saved.', user: { name: user.name, address: user.address } });
